...
const onlineGSList = document.querySelectorAll(".list-1 li");
const stoGSList = document.querySelectorAll(".list-2 li");

let onlineGSLocationData = [
  { lat: -13, lng: 20, msg: 'Globe Tower BGC, Taguig' },
  { lat: -8, lng: 28, msg: 'Puregold Parañaque'},
  { lat: -5, lng: 35, msg: 'Gogoro Experience Center Greenbelt 4' },
  { lat: -15, lng: 33, msg: 'Puregold Makati' },
  { lat: -20, lng: 34, msg: 'Phoenix Shaw Boulevard' },
];

let stoGSLocationData = [
  { lat: -10, lng: 20, msg: 'Globe Tower BGC, Taguig' },
  { lat: -3, lng: 25, msg: 'Puregold Parañaque'},
  { lat: -7, lng: 33, msg: 'Gogoro Experience Center Greenbelt 4' },
  { lat: -6, lng: 33, msg: 'Puregold Makati' },
  { lat: -22, lng: 34, msg: 'Phoenix Shaw Bouleletd' },
  { lat: -4, lng: 20, msg: 'Globe Tower BGC, Taguig' },
  { lat: -8, lng: 23, msg: 'Puregold Parañaque'},
  { lat: -5, lng: 37, msg: 'Gogoro Experience Center Greenbelt 4' },
  { lat: -9, lng: 19, msg: 'Puregold Makati' },
  { lat: -18, lng: 34, msg: 'Phoenix Shaw Bouleletd' },
  { lat: -15, lng: 31, msg: 'Puregold Makati' },
  { lat: -24, lng: 34, msg: 'Phoenix Shaw Bouleletd' },
];
...
let onlineGSMarkers = onlineGSLocationData.map(function(location) {
  ...
  .addTo(map)
  .bindPopup(location.msg, {
    closeButton: false
  });
});

let stoGSMarkers = stoGSLocationData.map(function(location) {
  return L.marker([location.lat, location.lng], {
...
onlineGSList.forEach(function(li, index) {
  li.addEventListener('mouseover', function() {
    map.setView(onlineGSMarkers[index].getLatLng(), 5);
    onlineGSMarkers[index].openPopup();
  });
});

stoGSList.forEach(function(li, index) {
  li.addEventListener('mouseover', function() {
    map.setView(stoGSMarkers[index].getLatLng(), 5);
    stoGSMarkers[index].openPopup();
  });
});
